...
const getRecommendedFeatures = () => {
  ...
  return Object.entries(pages)
    .filter(([key, page]) => {
      ...
      // 必须启用推荐
      if (!("enableRecommend" in typedPage) || !typedPage.enableRecommend)
        return false;
      // 排除下线功能
      ...
      // 只显示主页面,不显示子页面
      if ("group" in typedPage && typedPage.group && typedPage.group !== key)
        return false;
    ...
    .map(([key, page]) => ({
      ...
      description:
        ("shareTitle" in (page as TPage) && (page as TPage).shareTitle?.[0]) ||
        (page as TPage).title,
    }));
};
